...
// TrustyKeymaster implements handlers for IPC operations. Most operations are implemented by
// AndroidKeymaster but some operations which are not part of the interface with Android are
// implemented here. These operations are expected to be called from a bootloader or another Trusty
// application.
class TrustyKeymaster : public AndroidKeymaster {
  public:
    ...
    // AppendAttestationCertChain sets a single certificate in an attestation certificate chain.
    ...
    // AtapGetCaRequest is the first of two calls that are part of the the Android Things
